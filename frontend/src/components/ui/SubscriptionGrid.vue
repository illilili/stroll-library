    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn style="margin-left: 5px;" @click="purchaseSubscriptionDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('User')">
                ...
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="expireSubscriptionDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('System')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>구독 만료
                </v-btn>
                <v-dialog v-model="expireSubscriptionDialog" width="500">
                    <ExpireSubscription
                        @closeDialog="expireSubscriptionDialog = false"
            ...
            <SubscriptionHistory @search="search" style="margin-bottom: 10px; background-color: #ffffff;"></SubscriptionHistory>
        ...
        <v-col>
            ...
            >
                <v-card>
                    ...
                    <v-card-text>
                        <div>
                            <String label="SubscriptionId" v-model="selectedRow.subscriptionId" :editMode="true"/>
...
export default {
    ...
    data: () => ({
        ...
        expireSubscriptionDialog: false,
    }),
    watch: {
    },
    methods:{
        ...
        async expireSubscription(params){
            try{
                var path = "expireSubscription".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','ExpireSubscription 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
                }
                this.expireSubscriptionDialog = false
            }catch(e){
                console.log(e)
            }
        },
