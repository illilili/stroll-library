    ...
    <v-card outlined style="border-radius: 0; margin-top: 10px; padding-bottom: 10px; background-color: #F4F5FA;">
        ...
        <v-card-text style="margin-top:-10px;">
            <v-row no-gutters style="margin: 15px 0px -10px -15px;">
                ...
                <v-col style="max-width:140px;">
                    <Date class="attributes-list" label="RequestedAt" v-model="value.parameters.requestedAt" :editMode="editMode"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        name: 'PublicationRequestHistory',
        components:{
        },
        props: {},
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'contents/search/publicationRequestHistory',
                parameters: {}
            },
        }),
        created() {
